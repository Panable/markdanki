...
pub fn MDToDeck(arena: *std.heap.ArenaAllocator, filename: []const u8) !Deck {
    const alloc = arena.allocator();
    const cwd = std.Io.Dir.cwd();
    var threaded: std.Io.Threaded = .init_single_threaded;
    const io = threaded.io();
    const fileContents: []u8 = try cwd.readFileAlloc(io, filename, alloc, std.Io.Limit.limited(4096));
